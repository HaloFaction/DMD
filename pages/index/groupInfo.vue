<template>
    <scroll-view style="padding: 15px;box-sizing: border-box;">
		...
		<fix-window />
		<!-- #endif -->
		<view style="width: 100%;height: 100%;" class="padding ">
		...
		<uni-popup ref="priceshow" type="center">
			...
			<button class="bg-red" @click="showp()"><h4>P</h4></button>
			</view>
				...
				uni.getStorage({
					...
					success:function(res){
						...
						db.collection('xglb').where({
							...
							var list=[]
							for(var i=0;i<data.length;i++){
								list.push(data[i].occu+"-"+data[i].name)
							}
			...
			uni.showLoading({
				...
				uni.showModal({
					...
					success:function(res){	
						...
						if(res.confirm){
							db.collection("gift").where({_id:_this.giftid}).get().then(
								(res)=>{
									console.log(res.result.data)
									var user=res.result.data[0].puser
									user.push(_this.userInfo.username)
									db.collection("gift").where({_id:_this.giftid}).update({
										puser:user
									}).then(			
											db.collection('record').add({
												account:_this.userInfo.username,
												name:_this.user[_this.index],
												giftno:_this.giftid,
												price:"p",
											})			
									)
